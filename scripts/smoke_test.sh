...
python -m cfm.synth.generate --n-samples "${N_TRAIN}" --seed-base 20260630 \
    --out-dir "${OUT}/data/stage1/train" --shard-size "${SHARD}"
python -m cfm.synth.generate --n-samples "${N_VAL}" --seed-base 90000000 \
    --out-dir "${OUT}/data/stage1/val" --shard-size "${SHARD}"

echo "==> stage 2: depth-to-time conversion"
python -m cfm.timeconv.run --source-dir "${OUT}/data/stage1/train" \
...
python -m cfm.timeconv.run --source-dir "${OUT}/data/stage1/val" \
...
for split in train val; do
    python -m cfm.conditions.run --source-dir "${OUT}/data/stage1/${split}" \
        --time-axis "${OUT}/data/stage2/train/time_axis.json" \
